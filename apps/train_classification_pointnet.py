...
parser.add_argument('--rotate_axis', type=str, default='all',
                    help="Axis rotation when using augmentation on training/test. (possible values: 'x', 'y', 'z' and 'all')")
...
def evaluate(loader, classifier):
    total_correct = 0
    total_testset = 0
    for i, data in tqdm(enumerate(loader, 0)):
        ...
        for i in range(points.shape[0]):
            ...
            points[i] = points[i] @ torch.Tensor(lrf[0].T)
...
if opt.path_pointnet == '':
    ...
    for epoch in range(opt.nepoch):
        ...
        print(blue("Train acc: {}".format(right/len(dataset))))
else:
...
print("final accuracy {}".format(evaluate(testdataloader, classifier)))
